...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if cache_midas:  # if training
            m_gb = 0  # Gigabytes of cached midas images
...
def load_midas(self, index):
    ...
    if midas is None:  # not cached
        ...
        if r < 1 or (self.augment and r != 1):  # always resize down, only resize up if training with augmentation
            interp = cv2.INTER_AREA if r < 1 and not self.augment else cv2.INTER_LINEAR
            midas = cv2.resize(midas, (int(w0 * r), int(h0 * r)), interpolation=interp)
        return midas, (h0, w0), midas.shape[:2]  # img, hw_original, hw_resized
    else:
        return self.imgs[index], self.img_hw0[index], self.img_hw[index]  # img, hw_original, hw_resized
...
def load_mosaic(self, index):
    ...
    img4, labels4, midas4 = random_affine(img4, midas, labels4,
