...
class FlowBlock(nn.Module):
    ...
    def __init__(self, suffix, num_prev_channels=0):
...
class RefinementNet(nn.Module):
    ...
    def __init__(self, suffix):
        ...
        self.suffix = suffix
        self.refinement_block = RefinementBlockMask(suffix)
...
class RefinementNetConcat(nn.Module):
    ...
    def __init__(self, suffix):
        ...
        self.suffix = suffix
        self.refinement_block = RefinementBlockConcat(suffix)
...
class RefineModel(nn.Module):
    def __init__(self, suffix):
        ...
        if 'concat' in self.suffix:
            self.refinement_net = RefinementNetConcat(suffix)
        else:
            self.refinement_net = RefinementNet(suffix)
            pass

        W, H = 64, 48
        self.upsample = torch.nn.Upsample(size=(H, W), mode='bilinear')

        if 'crfrnn_only' in self.suffix:
            self.plane_to_depth = PlaneToDepth(normalized_K = True, W=256, H=192)
            self.crfrnn = CRFRNNModule(image_dims=(192, 256), num_iterations=5)
            pass
        return
